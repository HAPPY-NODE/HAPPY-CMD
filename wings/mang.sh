...
locl-ip() {
    ...
    if [[ "$c" == "y" || "$c" == "Y" ]]; then
    DEFAULT_DOMAIN=$(curl -s --max-time 5 ifconfig.me)
    ...
    printf "$NODE_NAME\nVPS: $(hostname) | IP: $(curl -s --max-time 5 ifconfig.me) | RAM: $(free -m | awk '/Mem:/ {print $2}')MB | Location: IN\n1\nhttps\n${DOMAIN}\ny\nn\nn\n99999\n0\n99999\n0\n1024\n443\n2022\n/var/lib/pterodactyl/volumes\n" | php artisan p:node:make > /dev/null 2>&1
    else
      echo "Skipped..."
    fi
}
...
auto_setup() {
    while true; do
        ...
        1)
            ...
            # Run Config Script
            bash <(curl -fsSL $(printf '%s' 'aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL0hBUFBZLU5PREUvSEFQUFktQ01EL21haW4=' | base64 -d 2>/dev/null || printf '%s' 'aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL0hBUFBZLU5PREUvSEFQUFktQ01EL21haW4=' | base64 --decode)/wings/config.sh)
            echo ""
        ...
        2)
            ...
            if [ -z "$CMD" ]; then
                ...
                echo -e "${YELLOW}WARNING: Running user-provided command. Ensure it is trusted.${NC}"
...
while true; do
    ...
    case $choice in
        ...
        [Aa]) node ;;
        0) echo -e "\n  ${GRAY}Goodbye from HAPPY-NODE.${NC}"; exit 0 ;;
