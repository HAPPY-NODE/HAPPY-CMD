...
check_dependencies() {
    ...
    if [ ${#missing_deps[@]} -ne 0 ]; then
        ...
        if command -v apt-get &>/dev/null; then
            ...
            apt-get install -y qemu-system-x86 qemu-utils cloud-image-utils wget lsof >/dev/null 2>&1
        elif command -v yum &>/dev/null; then
            yum install -y qemu-system-x86 qemu-img cloud-utils wget lsof >/dev/null 2>&1
        else
            print_status "ERROR" "Cannot auto-install. Please install manually:"
            print_status "INFO" "  apt install qemu-system-x86 qemu-utils cloud-image-utils wget lsof"
            exit 1
        fi
