...
check_dependencies() {
    ...
    if [ ${#missing_deps[@]} -ne 0 ]; then
        echo
        print_status "INFO" "Auto-installing missing dependencies..."
        if command -v apt-get &>/dev/null; then
            apt-get update -y -q >/dev/null 2>&1
            apt-get install -y qemu-system-x86 qemu-utils cloud-image-utils wget openssl net-tools uuid-runtime >/dev/null 2>&1
        elif command -v yum &>/dev/null; then
            yum install -y qemu-system-x86 qemu-img cloud-utils wget openssl net-tools uuid >/dev/null 2>&1
        else
            print_status "ERROR" "Cannot auto-install. Please install manually:"
            echo -e "  Ubuntu/Debian: sudo apt install qemu-system cloud-image-utils wget openssl"
            echo -e "  Fedora:        sudo dnf install qemu-system-x86 cloud-utils wget openssl"
            exit 1
        fi
