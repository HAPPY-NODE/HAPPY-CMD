...
hdr() {
    ...
    echo -e "  ${CB}│${Y}     ██║  ██║██║  ██║╚██████╔╝██║            ${CB}│${NC}"
    echo -e "  ${CB}│${Y}     ╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝ ╚═╝            ${CB}│${NC}"
    echo -e "  ${CB}│${DG}       HAPPY  NODE  •  HARDWARE  MODE          ${CB}│${NC}"
    echo -e "  ${CB}╰──────────────────────────────────────────────╯${NC}"
    local kvm_st="${G}ACTIVE${NC}"
    [ -e /dev/kvm ] || kvm_st="${R}MISSING${NC}"
    local total_vms=$(find "$VM_DIR" -name "*.conf" 2>/dev/null | wc -l)
    local run_vms=$(pgrep -fc "qemu-system" 2>/dev/null || echo 0)
    echo -e "  ${DG}KVM:${NC} $kvm_st  ${DG}│${NC}  ${DG}VMs:${NC} ${G}${run_vms}${NC}${DG}/${NC}${total_vms}  ${DG}│${NC}  ${DG}Dir:${NC} ${VM_DIR}"
}
