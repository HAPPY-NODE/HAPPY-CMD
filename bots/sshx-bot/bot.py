...
# Clean ANSI escape codes from sshx output
def clean_ansi(text):
    ansi_escape = re.compile(r'\x1b\[[0-9;]*[a-zA-Z]|\x1b\].*?\x07')
    return ansi_escape.sub('', text).strip()

# Extract URL from sshx output
def extract_url(text):
    cleaned = clean_ansi(text)
    match = re.search(r'https?://sshx\.io/\S+', cleaned)
    if match:
        return match.group(0)
    return cleaned

# Button view for opening link
class LinkButton(discord.ui.View):
    def __init__(self, url):
        super().__init__()
        self.url = url
        button = discord.ui.Button(label="Open Link", style=discord.ButtonStyle.link, url=url)
...
VPS_HOSTNAME = os.getenv('VPS_HOSTNAME', 'happy-node')  # Base hostname, append user ID
...
def get_server_limit():
    return int(os.getenv('SERVER_LIMIT', 3))
...
async def async_install_sshx(container_id, os_type):
    install_cmd = "apt-get update && apt-get install -y curl && curl -sSf https://sshx.io/get | sh"
    try:
        proc = await asyncio.create_subprocess_exec(
...
async def capture_ssh_session_line(process):
    async def read_stream(stream, name):
        while True:
            try:
                output = await asyncio.wait_for(stream.readline(), timeout=60.0)
                if not output:
                    break
                output = output.decode('utf-8').strip()
                logger.info(f"sshx {name}: {output}")
                if "sshx.io/" in output:
                    return output
            except asyncio.TimeoutError:
                break
        return None
...
async def docker_exec_sshx(container_id):
    try:
        exec_cmd = await asyncio.create_subprocess_exec(
            "docker", "exec", container_id, "/usr/local/bin/sshx",
...
async def regen_ssh_command(interaction: discord.Interaction, vps_identifier, send_response=True, target_user=None):
    ...
    if exec_process:
        ...
        if ssh_line:
            ...
            link = extract_url(ssh_line)
            embed = discord.Embed(title="New SSH Session Generated", description=f"**Click below to open VPS:**\n{link}", color=discord.Color.green(), timestamp=datetime.now(timezone.utc))
...
async def create_vps(interaction: discord.Interaction, os_type, ram=DEFAULT_RAM, cpu=DEFAULT_CPU, disk=DEFAULT_DISK, target_user=None):
    ...
    if is_banned(user_id):
        ...
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
    if count_user_vps(user_id) >= get_server_limit():
        embed = discord.Embed(description=f"You have reached the limit of {get_server_limit()} VPS instances.", color=discord.Color.red())
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
...
async def admin_vps_info(interaction: discord.Interaction, target_user: discord.User, vps_identifier: str):
    ...
    if vps['ssh_command']:
        link = extract_url(vps['ssh_command'])
        embed.add_field(name="SSH Link", value=link, inline=False)
    embed.set_footer(text=WATERMARK, icon_url=bot.user.avatar.url if bot.user.avatar else None)
    await interaction.response.send_message(embed=embed)
...
async def about(interaction: discord.Interaction):
    embed = discord.Embed(
        title="HAPPY NODE",
        description="Free VPS hosting powered by Docker. Create, manage, and control VPS instances directly from Discord.",
    ...
    embed.add_field(
        name="Features",
        value=(
            "`/deploy` - Create a free VPS instance\n"
            "`/start` `/stop` `/restart` - Control your VPS\n"
            "`/regen-ssh` - Get new access link\n"
            "`/vps-info` - View VPS details\n"
    ...
    embed.add_field(
        ...
        value=(
            ...
            "RAM: 2GB | CPU: 1 Core | Disk: 5GB\n"
            "Access: Browser-based terminal via sshx"
        ),
        inline=False
    )
    ...
    embed.add_field(
        name="Links",
        value=(
            "💻 **GitHub:** [HAPPY-NODE](https://github.com/HAPPY-NODE)\n"
            "🌐 **Toolkit:** [HAPPY-CMD](https://github.com/HAPPY-NODE/HAPPY-CMD)"
        ),
        inline=False
    )

    embed.set_footer(
        text="Powered by HAPPY NODE",
        icon_url="https://i.postimg.cc/jdbphsXP/Chat-GPT-Image-Sep-23-2026-05-48-16-PM.png"
    )
    embed.set_thumbnail(
        url="https://i.postimg.cc/jdbphsXP/Chat-GPT-Image-Sep-23-2026-05-48-16-PM.png"
    )
