...
_raw_main_admin_ids = os.getenv('MAIN_ADMIN_ID', '0')
...
BOT_DEVELOPER = os.getenv('BOT_DEVELOPER', 'HAPPY-NODE')
...
async def delete_vps(ctx, user: discord.Member, vps_number: int, *, reason: str = "No reason"):
    ...
    # 4️⃣ Success embed
    embed = create_success_embed("🌟 HAPPY NODE - VPS Deleted Successfully")
...
async def user_info(ctx, user: discord.Member):
    ...
    embed.set_footer(text="HAPPY NODE • User Resource Dashboard")
    embed.timestamp = ctx.message.created_at
...
async def server_stats(ctx):
    ...
    embed.set_footer(text="HAPPY NODE • Real-Time Monitoring")
    embed.timestamp = ctx.message.created_at
