...
prompt_env() {
    ...
    if ! [[ "$MAIN_ADMIN_ID" =~ ^[0-9]+(,[0-9]+)*$ ]]; then
        st ERR "Admin ID must be numeric (comma-separated IDs allowed)"
        return 1
    fi
    ...
    if [ -n "$bid" ] && [ "$bid" = "${MAIN_ADMIN_ID%%,*}" ]; then
        st WARN "Admin ID $bid is the BOT's own ID — bot will IGNORE every command!"
...
SyslogIdentifier=happy-svm-bot
