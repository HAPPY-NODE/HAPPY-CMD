...
if [ $HAS_DB -eq 1 ]; then
    ...
    # If DB exists, reuse existing creds
    DB_NAME="convoy"; DB_USER="convoy_user"
    DB_PASS=$(grep -m1 '^DB_PASSWORD=' /var/www/convoy/.env 2>/dev/null | cut -d= -f2-)
    [ -z "$DB_PASS" ] && DB_PASS=$(openssl rand -base64 12 | tr -d '/+=' | cut -c1-20)
fi
