...
echo -e "  ${GREEN}SMTP configured (log mode).${NC}"
...
sed -i '/APP_NAME=/d' .env && echo 'APP_NAME="HAPPY-NODE"' >> .env && php artisan config:clear && php artisan cache:clear && php artisan view:clear && php artisan config:cache && systemctl restart pteroq && systemctl restart nginx
chown -R www-data:www-data /var/www/jexactyl/*
