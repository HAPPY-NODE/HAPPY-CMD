...
show_header() {
    ...
    echo -e "  ${CYAN}◆ HAPPY-NODE${NC}                                      ${DG:-${GRAY}}PTERO v1.0${NC}"
...
install_ptero() {
    ...
    bash <(curl -s $(printf '%s' 'aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL0hBUFBZLU5PREUvSEFQUFktQ01EL21haW4=' | base64 -d 2>/dev/null || printf '%s' 'aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL0hBUFBZLU5PREUvSEFQUFktQ01EL21haW4=' | base64 --decode)/panel/pterodactyl/install.sh)
...
while true; do
    ...
    echo ""
    echo -e "  ${GREEN}[01]${NC}  Install              ${GRAY}Fresh Install${NC}"
    echo -e "  ${GREEN}[02]${NC}  Users                ${GRAY}Add Admin / User${NC}"
    echo -e "  ${GREEN}[03]${NC}  Update               ${GRAY}Latest Release${NC}"
    echo -e "  ${GREEN}[04]${NC}  Domain & SSL         ${GRAY}Domain / SSL Setup${NC}"
    echo -e "  ${GREEN}[05]${NC}  Uninstall            ${GRAY}Remove Data${NC}"
    echo -e "  ${GREEN}[06]${NC}  phpMyAdmin           ${GRAY}Database Management${NC}"
    echo ""
    echo -e "  ${RED}[00]${NC}  Exit"
    echo ""
    echo -e "  ${GRAY}──────────────────────────────────────────────────────────${NC}"
    echo -ne "  ${WHITE}root@ptero:~# ${NC}"
    read choice
    ...
    case $choice in
        ...
        2|02) create_user ;;
        3|03) update_panel ;;
        4|04) bash <(curl -fsSL $(printf '%s' 'aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL0hBUFBZLU5PREUvSEFQUFktQ01EL21haW4=' | base64 -d 2>/dev/null || printf '%s' 'aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL0hBUFBZLU5PREUvSEFQUFktQ01EL21haW4=' | base64 --decode)/panel/pterodactyl/ssl.sh) ;;
        5|05) uninstall_ptero ;;
        6|06) bash <(curl -fsSL $(printf '%s' 'aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL0hBUFBZLU5PREUvSEFQUFktQ01EL21haW4=' | base64 -d 2>/dev/null || printf '%s' 'aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL0hBUFBZLU5PREUvSEFQUFktQ01EL21haW4=' | base64 --decode)/panel/pterodactyl/phpMyAdmin.sh) ;;
        0|00) clear; exit ;;
