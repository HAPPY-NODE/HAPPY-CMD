...
apt -y install php8.3 php8.3-{common,cli,gd,mysql,mbstring,bcmath,xml,fpm,curl,zip,intl,redis,tokenizer,ctype,simplexml,dom} mariadb-server nginx tar unzip git redis-server
...
DB_NAME="paymenter"
DB_USER="paymenter"
ask "Database Password (leave blank to generate random)" "" DB_PASS
if [ -z "$DB_PASS" ]; then
    DB_PASS=$(openssl rand -base64 16)
    echo -e "  ${GREEN}Generated random password: ${WHITE}$DB_PASS${NC}"
fi
...
DB::table('settings')->updateOrInsert(['key'=>'company_name'], ['value'=>'HAPPY-NODE']);
